...
*   **Principio Teorico:** Minimizzare il residuo equivale geometricamente a imporre che il residuo stesso sia ortogonale allo spazio generato dalle colonne di $A$. Ciò porta al sistema quadrato ($n \times n$):
    $$(A^T A)x = A^T b$$
*   **Condizione di Esistenza:** La matrice quadrata $A^T A$ è simmetrica e definita positiva se e solo se la matrice $A$ ha rango massimo ($\operatorname{rank}(A) = n$, ovvero colonne linearmente indipendenti). In tal caso, si applica la fattorizzazione di **Cholesky** ($LL^T$) per risolvere il sistema in modo efficiente.
